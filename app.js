...
app.get("/", (req, res) => { // アクセス要求があったら 
 res.sendFile(__dirname + "/index.html"); // クライアントに index.html を送る 
}); 
// https サーバを立てる 
...
console.log("サーバ起動（終了は Ctrl+C）"); // サーバ起動成功の表示 
// 機能(B) 双方向通信の処理 
const io = require("socket.io")(svr); // socket.io モジュールを使う ② 
io.on("connection", (socket) => { // 接続(connection)があったら ③ 
 console.log("ユーザが接続"); // ユーザ接続時の表示 
 // (B1) 送信者を含む全員向けの配信の処理 
 socket.on("all", (data) => { // all という名の socket が届いたら ④ 
 console.log("受信 (all)"); // 受信の表示 
 io.emit("msg_all", data); // msg_all という名で全員に data 送信 ⑤ 
 }); 
 // (B2) 送信者以外への配信の処理 
 socket.on("others", (data) => { // others という名の socket が届いたら ⑥ 
 console.log("受信 (others)"); // 受信の表示 
 socket.broadcast.emit("msg_others", data); // msg_others という名で送信者以外に data 送信 ⑦ 
 }); 
});